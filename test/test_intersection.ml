...
type intersection_state = {
  ...
  y : int ref;
  angle : float ref;
  settings : Tool_types.settings;
}

type app_state = {
  intersections : intersection_state list ref;
  selected : int option ref;  (* Index of selected intersection *)
  drag_start : (float * float) option ref;  (* Mouse position when drag started *)
  drag_offset : (float * float) option ref;  (* Offset from intersection center when dragging *)
...
let () =
  ...
  let window = GWindow.window 
    ~title:"Interactive Intersection Test - Click to select, Drag to move, Use rotate button to rotate" 
    ~width:600 
    ~height:500 
    ~position:`CENTER () in
  ...
  ignore (drawing_area#misc#connect#draw ~callback:(fun cr ->
    ...
    List.iteri (fun idx intersection ->
      (* Draw the intersection *)
      Intersection.Intersection.draw cr 
        ~x:!(intersection.x) 
        ~y:!(intersection.y) 
      ...
      if !(state.selected) = Some idx then begin
        Intersection.Intersection.draw_selection cr 
          ~x:!(intersection.x) 
          ~y:!(intersection.y) 
          ~angle:!(intersection.angle)
          intersection.settings;
    ...
    Cairo.show_text cr "Click intersection to select | Drag to move | Use rotate button to rotate";
  ...
  ignore (drawing_area#event#connect#button_press ~callback:(fun ev ->
    ...
    if button = 1 then begin (* Left mouse button *)
      ...
      List.iteri (fun idx intersection ->
        if !clicked_intersection = None then begin
          ...
          if !(state.selected) = Some idx then begin
            if Intersection.Intersection.point_on_rotate_button 
                 ~x:!(intersection.x) 
                 ~y:!(intersection.y) 
                 ~angle:!(intersection.angle)
                 ~px:x ~py:y 
          ...
          if !clicked_intersection = None then begin
            if Intersection.Intersection.point_inside 
                 ~x:!(intersection.x) 
                 ~y:!(intersection.y) 
                 ~px:x ~py:y 
                 settings then begin
  ...
  ignore (drawing_area#event#connect#button_release ~callback:(fun _ev ->
  ...
  ignore (drawing_area#event#connect#motion_notify ~callback:(fun ev ->
    if !(state.button_pressed) && !(state.selected) <> None then begin
      ...
      let selected_idx = match !(state.selected) with
        | Some idx -> idx
        | None -> assert false
      in
      ...
      if !(state.rotating) then begin
        ...
        let new_angle = Intersection.Intersection.calculate_rotation ~cx ~cy ~mx:x ~my:y in
  ...
  drawing_area#event#add [`BUTTON_PRESS; `BUTTON_RELEASE; `POINTER_MOTION; `POINTER_MOTION_HINT];
