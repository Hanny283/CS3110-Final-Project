...
module type TOOL = sig 
    ...
    val draw_selection : Cairo.context -> x:int -> y:int -> angle:float -> settings -> unit

    (* Draw rotate button (circular arrow icon) on the right side of the tool *)
    val draw_rotate_button : Cairo.context -> x:int -> y:int -> angle:float -> settings -> unit
    ...
    val point_on_rotate_button : x:int -> y:int -> angle:float -> px:float -> py:float -> settings -> bool
